...
public class RAGOptOutTest {

	public final static Logger LOGGER = Logger.getLogger(RAGOptOutTest.class);
	...
	public void doGet() {
		...
		try {
			...
			when(ragOfferService.getRagOptOut(any(JSONObject.class))).thenReturn(synchOffersObj);
			ragOptOut.doGet(request, response);
		} catch (Exception e) {
			LOGGER.error(e);
		}
	}
