...
public class SAGOptOutTest {

	public final static Logger LOGGER = Logger.getLogger(SAGOptOutTest.class);
	...
	public void doGet() {
		...
		try {
			...
			when(sagOfferService.getSagOptOut(any(JSONObject.class))).thenReturn(synchOffersObj);
			sagOptOut.doGet(request, response);
