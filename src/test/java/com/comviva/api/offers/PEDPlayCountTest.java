...
public class PEDPlayCountTest {

	public final static Logger LOGGER = Logger.getLogger(PEDPlayCountTest.class);
	...
	@InjectMocks
	private PEDPlayCount pedPlayCount;
	...
	public void doGet() {
		...
		objectInArray1.put(OFFER_ID, "10001");
		objectInArray1.put(OFFER_DESC, "Morning Offers");
		JSONObject objectInArray2 = new JSONObject();
		objectInArray2.put(OFFER_ID, "10001");
		objectInArray2.put(OFFER_DESC,"Evening Offers");
		offerArray.put(objectInArray1);
		offerArray.put(objectInArray2);
		...
		actOfferResObj.put(OFFERS, offerArray);
		...
		try {
			...
			when(pedprocessService.getAvailablePlays(any(JSONObject.class))).thenReturn(synchOffersObj);
			pedPlayCount.doGet(request, response);
		} catch (Exception e) {
			LOGGER.error(e);
		}
	}
