...
public class PEDPlayHistoryTest {

	public final static Logger LOGGER = Logger.getLogger(PEDPlayHistoryTest.class);
	...
	@InjectMocks
	private PEDPlayHistory pedPlayHistory;
	...
	public void doGet() {
		...
		try {
			when(pedprocessService.getPlayHistory(any(JSONObject.class))).thenReturn(synchOffersObj);
			Mockito.doNothing().when(updaterDAO).insertApiLog(Mockito.any(ApiLog.class));
			pedPlayHistory.doGet(request, response);
		} catch (Exception e) {
			LOGGER.error(e);
		}
	}
