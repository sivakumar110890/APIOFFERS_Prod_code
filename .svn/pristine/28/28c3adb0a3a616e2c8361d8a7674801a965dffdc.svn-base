...
public class RAGGetOfferTest {

	public final static Logger LOGGER = Logger.getLogger(RAGGetOfferTest.class);
	...
	@InjectMocks
	private RAGGetOffer ragGetOffer;
	...
	public void doGet() {
		...
		try {
			...
			when(ragOfferService.RagGetOfferInfo(any(JSONObject.class))).thenReturn(synchOffersObj);
			ragGetOffer.doGet(request, response);
		} catch (Exception e) {
			LOGGER.error(e);
		}
	}
