...
public class PEDRedeemPlayTest {

	public final static Logger LOGGER = Logger.getLogger(PEDRedeemPlayTest.class);
	...
	@InjectMocks
	private PEDRedeemPlay pedRedeemPlay;
	...
	public void doGet() {
		...
		try {
			when(pedprocessService.redeemPlay(any(JSONObject.class))).thenReturn(synchOffersObj);
			Mockito.doNothing().when(updaterDAO).insertApiLog(Mockito.any(ApiLog.class));
			pedRedeemPlay.doGet(request, response);
		} catch (Exception e) {
			LOGGER.error(e);
		}
	}
