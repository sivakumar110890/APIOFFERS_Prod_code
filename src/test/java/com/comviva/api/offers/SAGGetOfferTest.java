...
public class SAGGetOfferTest {

	public final static Logger LOGGER = Logger.getLogger(SAGGetOfferTest.class);
	...
	@InjectMocks
	private SAGGetOffer sagGetOffer;
	...
	public void doGet() {
		...
		try {
			when(sagOfferService.SagGetOfferInfo(any(JSONObject.class))).thenReturn(synchOffersObj);
			Mockito.doNothing().when(updaterDAO).insertApiLog(Mockito.any(ApiLog.class));
			sagGetOffer.doGet(request, response);
		} catch (Exception e) {
			LOGGER.error(e);
		}
	}
